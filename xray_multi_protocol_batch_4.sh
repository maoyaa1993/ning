#!/bin/bash
# Xray VMESS+KCP+DTLS 和 SOCKS5 批量部署脚本 (修复版)
# 修复内容：IP端口对应关系、VMESS链接格式、SOCKS5认证信息
...
# 使用用户提供的IP列表，确保端口与IP正确对应
IP_ADDRESSES=(
    "2407:cdc0:b007:7da9:6797:6748:601d:5c31"
    "2407:cdc0:b007:6a67:2cc:5fb:4bb7:1b5a"
    "2407:cdc0:b007:1382:36a7:658b:3446:4274"
    "2407:cdc0:b007:5646:7a38:1c5e:608b:335"
    "2407:cdc0:b007:5910:1dd1:7a74:3c87:25f7"
)
...
config_vmess_kcp_dtls() {
    ...
    for ((i = 0; i < ${#IP_ADDRESSES[@]}; i++)); do
        DTLS_PASSWORDS[$i]=$(generate_random_string 10)
    done
...
EOF
    ...
    for ((i = 0; i < ${#IP_ADDRESSES[@]}; i++)); do
        ...
        echo "        \"security\": \"none\"," >> /usr/local/etc/xray/config.json
        echo "        \"kcpSettings\": {" >> /usr/local/etc/xray/config.json
        echo "          \"mtu\": 1350," >> /usr/local/etc/xray/config.json
        echo "          \"tti\": 20," >> /usr/local/etc/xray/config.json
        echo "          \"uplinkCapacity\": 5," >> /usr/local/etc/xray/config.json
        echo "          \"downlinkCapacity\": 20," >> /usr/local/etc/xray/config.json
        ...
        echo "            \"password\": \"${DTLS_PASSWORDS[$i]}\"" >> /usr/local/etc/xray/config.json
        echo "          }" >> /usr/local/etc/xray/config.json
        echo "        }" >> /usr/local/etc/xray/config.json
        echo "      }," >> /usr/local/etc/xray/config.json
...
EOF
    ...
    for ((i = 0; i < ${#IP_ADDRESSES[@]}; i++)); do
        # 为每个节点生成唯一的用户名和密码
        SOCKS5_USER[$i]="panhu$((i+1))"
        SOCKS5_PASS[$i]="panhu666KK$((i+1))"
...
generate_vmess_link() {
    ...
    for ((i = 0; i < ${#IP_ADDRESSES[@]}; i++)); do
        # 确保使用正确的IP地址格式
        LINK="vmess://$(echo -n "{\"v\":\"2\",\"ps\":\"${USER_NAME[$i]}\",\"add\":\"${IP_ADDRESSES[$i]}\",\"port\":\"${VMESS_PORT[$i]}\",\"id\":\"${USER_UUID[$i]}\",\"aid\":\"0\",\"net\":\"kcp\",\"type\":\"dtls\",\"tls\":\"none\"}" | base64 -w 0)"
...
menu() {
    clear
    echo "##################################################################"
    echo -e "#                   ${RED}Xray 批量部署脚本 (修复版)${PLAIN}                              #"
